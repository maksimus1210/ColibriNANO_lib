...
bool LibLoader::load(const wstring &file)
{
    ...
    m_initialize = reinterpret_cast<pFunc1>(GetProcAddress(hDLL, "initialize"));
    if (m_initialize == nullptr)
        return false;

    m_finalize = reinterpret_cast<pFunc1>(GetProcAddress(hDLL, "finalize"));
    if (m_finalize == nullptr)
        return false;

    m_version = reinterpret_cast<pVersion>(GetProcAddress(hDLL, "version"));
    if (m_version == nullptr)
        return false;

    m_information = reinterpret_cast<pInformation>(GetProcAddress(hDLL, "information"));
    if (m_information == nullptr)
        return false;

    m_devices = reinterpret_cast<pDevices>(GetProcAddress(hDLL, "devices"));
    ...
    m_close = reinterpret_cast<pClose>(GetProcAddress(hDLL, "close"));
    if (m_close == nullptr)
        return false;

    m_start = reinterpret_cast<pStart>(GetProcAddress(hDLL, "start"));
    if (m_start == nullptr)
        return false;
    ...
    m_setPreamp = reinterpret_cast<pSetPreamp>(GetProcAddress(hDLL, "setPream"));
    if (m_setPreamp == nullptr)
        return false;

    m_setFrequency = reinterpret_cast<pSetFrequency>(GetProcAddress(hDLL, "setFrequency"));
